...
def get_word_with_least_vowels(input):
    ...
    return inp[f] # replace this with right code. Use min builtin and define a new function to pass as key.
...
def get_min_max_words(input):
    ...
    minimum=min(k)
    maximum=max(k)
    mini=k.index(minimum)
    maxi=k.index(maximum)
    return(inp[mini],inp[maxi])
